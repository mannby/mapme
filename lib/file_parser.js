...
class FileParser {
  ...
  reportCycle(rootNode, parentNode, displayName) {
    ...
    console.log('CYCLE STRAND:')
    this.logStrand(parentNode, rootNode)
  ...
  createChildren(parentNode, rootNode, filesList, dir) {
    ...
    filesList.forEach(key => {
      ...
      var file = t.path
      var jsFile = file.endsWith('.js') || file.endsWith('.json') ? file : file + '.js'
      ...
      if (fs.existsSync(jsFile)) {
        ...
        if (fs.existsSync(jsDir)) {
          ...
          if (key.startsWith('.') || key.startsWith('/') || key.startsWith('~')) {
            console.log('Suspicious: ' + dir + ' ' + key)
          } else {
            if (!modules.has(key)) {
              modules.add(key)
              if (displayModules || (key.indexOf('/') > -1)) {
                console.log('Module? ' + dir + ' ' + key)
              }
            }
          }
        }
